...
CREATE TABLE `allergies` (
  ...
  `allergyinfo` varchar(344) NOT NULL,
...
INSERT INTO `allergies` (`allergiesId`, `allergyinfo`, `mealId`) VALUES
(3, 'Eggs', 2);
...
INSERT INTO `orderItems` (`orderItemId`, `orderId`, `mealId`, `drinkId`, `quantity`, `SpecialRequirements`) VALUES
(1, 1, 4, 5, 1, 'No sauce'),
(3, 1, 4, 5, 1, 'No sauce'),
(4, 1, 1, 3, 2, 'No onions'),
(5, 1, 1, 5, 1, 'No onions'),
(8, 1, 1, 5, 1, 'No onions'),
(9, 1, 1, 5, 1, 'No onions'),
(10, 1, 1, 5, 1, 'No onions'),
(11, 1, 1, 5, 1, 'No onions'),
(12, 1, 1, 5, 1, 'No onions'),
(13, 1, 1, 5, 1, 'No onions');
...
CREATE TABLE `preOrder` (
  ...
  `date` date NOT NULL,
  `mealId` int(11) NOT NULL,
  `TypeOfMeal` varchar(500) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `preOrder` (`preOrderId`, `CustomerId`, `time`, `status`, `date`, `mealId`, `TypeOfMeal`) VALUES
(1, 4, '12:00:00', 'cancelled', '2024-05-10', 1, 'frozen'),
(2, 4, '12:00:00', 'Pending', '2024-05-10', 2, 'frozen'),
(3, 5, '12:00:00', 'Pending', '2024-05-10', 2, 'frozen'),
(4, 5, '12:00:00', 'Pending', '2024-05-10', 2, 'frozen'),
(5, 5, '12:00:00', 'Pending', '2024-05-10', 2, 'frozen');
...
CREATE TABLE `preOrderItem` (
  ...
  `preOrderitemdetailId` int(11) NOT NULL,
  `mealId` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `preOrderItem` (`preOrderId`, `quantity`, `SpecialRequest`, `preOrderitemdetailId`, `mealId`) VALUES
(1, 1, 'No onions', 1, 4),
(1, 1, 'No onions', 2, 4),
(2, 1, 'No onions', 3, 4);
...
(19, 'Steak', 1, '10 minutes', '1 hour', 'To cook a great steak, start by seasoning it with salt and pepper. Heat a skillet or grill over medium-high heat and cook the steak for 3-4 minutes on each side for medium-rare. Let it rest for a few minutes before slicing', 2);
...
INSERT INTO `reservation` (`reservationId`, `CustomerId`, `date`, `time`, `numberOfGuests`, `tableId`, `status`) VALUES
(3, 5, '2024-05-10', '12:00:00', 4, 1, 'cancelled'),
(4, 5, '2024-05-12', '12:00:00', 4, 1, 'pending'),
(6, 5, '2024-05-13', '22:00:00', 5, 1, 'pending');
...
(3, 'anth', 'Sorrengial', 'waiter', '7790887', 'Block f Flat 6 Triq Patri Mosta');
...
INSERT INTO `StaffAccounts` (`StaffAccountid`, `staffId`, `email`, `password`) VALUES
(1, 1, 'xaden@lol.com', '$2y$10$Bg8xe7.7y7aMqRrD3fHU2OZ20x4TBAKDHUoBnW6ACi.qj51RWmsn.'),
(3, 3, 'anth@lol.com', 'lol');
...
ALTER TABLE `preOrderItem`
  ADD PRIMARY KEY (`preOrderitemdetailId`),
  ADD KEY `mealId` (`mealId`),
...
--
ALTER TABLE `allergies`
  MODIFY `allergiesId` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
ALTER TABLE `preOrder`
  MODIFY `preOrderId` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT for table `preOrderItem`
--
ALTER TABLE `preOrderItem`
  MODIFY `preOrderitemdetailId` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
ALTER TABLE `StaffAccounts`
  MODIFY `StaffAccountid` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
ALTER TABLE `allergies`
  ADD CONSTRAINT `allergies_ibfk_1` FOREIGN KEY (`mealId`) REFERENCES `meal` (`mealId`) ON UPDATE CASCADE;
...
--
-- Constraints for table `preOrderItem`
--
ALTER TABLE `preOrderItem`
  ADD CONSTRAINT `preorderitem_ibfk_1` FOREIGN KEY (`mealId`) REFERENCES `meal` (`mealId`) ON UPDATE CASCADE,
  ADD CONSTRAINT `preorderitem_ibfk_2` FOREIGN KEY (`preOrderId`) REFERENCES `preOrder` (`preOrderId`) ON UPDATE CASCADE;
...
--
ALTER TABLE `reservation`
  ADD CONSTRAINT `reservation_ibfk_1` FOREIGN KEY (`CustomerId`) REFERENCES `customer` (`customerId`) ON UPDATE CASCADE,
  ADD CONSTRAINT `reservation_ibfk_2` FOREIGN KEY (`tableId`) REFERENCES `RestaurantTable` (`tableId`) ON UPDATE CASCADE;

--
-- Constraints for table `StaffAccounts`
--
ALTER TABLE `StaffAccounts`
  ADD CONSTRAINT `staffaccounts_ibfk_1` FOREIGN KEY (`staffId`) REFERENCES `staff` (`staffId`) ON UPDATE CASCADE;
